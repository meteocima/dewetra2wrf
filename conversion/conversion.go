...
func ToWRFDA(obs sensor.Observation) string {
	...
	thirstLine :=
		dataQCError(num(obs.Metric.Pressure, 12.3), 1.0) +
			dataQCError(num(obs.Metric.WindspeedAvg, 12.3), 1.0) + //dataQCError(num(obs.Metric.WindspeedAvg, 12.3), 1.0) +
			dataQCError(num(obs.WinddirAvg, 12.3), 3.0) + //dataQCError(num(obs.WinddirAvg, 12.3), 3.0) +
			space(11) +
